...
- ✅ 로그인(`login`)에 성공하면 매장의 `last_login_at`이 현재 시각으로 갱신되고 `login_count`가 1 증가한다. (`register`는 로그인 횟수에 포함되지 않는다.)
...
- ✅ 소셜 로그인에 성공하면 고객의 `last_login_at`이 현재 시각으로 갱신되고 `login_count`가 1 증가한다(최초 로그인으로 계정이 생성되는 경우 `login_count`는 1로 시작한다).
