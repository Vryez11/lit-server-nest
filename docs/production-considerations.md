...
Dashboard 1차 구현은 현재 DB 상태를 기준으로 실시간 조회합니다.
이 방식은 데이터 정합성 확인이 쉽지만, 예약/결제 데이터가 크게 늘어나면 날짜 범위 조회에서 DB 부하가 커질 수 있습니다.

2차 구현에서는 `daily_statistics`를 운영 집계 테이블로 사용하도록 고도화해야 합니다.

권장 방향:

```txt
예약/결제 상태 변경
  -> 같은 트랜잭션 또는 이벤트 핸들러에서 daily_statistics 갱신
  -> Dashboard 날짜별 추이는 daily_statistics 조회
  -> 오늘 진행 중 상태처럼 즉시성이 필요한 값만 실시간 보정
```

추가로 필요한 작업:

- 예약 생성/승인/거절/체크인/완료/취소 시 일별 예약 카운트 갱신
- 결제 성공/취소/환불 시 일별 매출 갱신
- 보관함 점유율 산정 기준 확정 후 `occupancy_rate` 갱신
- 누락 또는 오류 집계를 복구하는 rebuild 배치 스크립트 추가
- `daily_statistics` 값과 원천 테이블 값을 비교하는 운영 점검 쿼리 추가
- 대시보드 날짜 범위가 길어질 경우 캐시 또는 읽기 전용 replica 사용 검토
