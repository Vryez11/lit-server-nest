...
export class AddressesController {
  ...
  @Get('search')
  @ApiOperation({
    summary: '주소를 검색합니다 (자동완성용).',
    description:
      '카카오 Local API를 호출하여 주소 검색 결과 + 좌표를 한 번에 반환합니다. 회원가입 등 인증 전에도 호출 가능합니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      '카카오 Local API를 호출하여 주소 문자열을 좌표로 변환합니다. 검색 API 결과에 좌표가 이미 포함되어 있으므로 보통은 검색 API만으로 충분합니다.',
  })
